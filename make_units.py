...
def get_string(string: str):
    """
    Return a string's contents iff it starts and ends with ' or ".
    """
    for j in '"', "'":
        if string.startswith(j) and string.endswith(j):
            return string[1:-1]
...
def transmogrify_shorthand(text: str):
    ...
    for match in unit_matches:
        ...
        tags = {}  # type: dict[str, str | list[str]]
        info = []
        ...
        unitdefs = filter(lambda x: x, [
            g['expr'],
            repr(names),
            repr(symbols) if symbols else None,
            "info=" + repr(" ".join(info)) if info else None,
            "tags=" + repr(tags) if tags else None,
        ])
        
        new_text += "{} = Unit(\n{}\n)\n".format(
            ' = '.join(names + symbols),
            '\n'.join(map('    {},'.format, unitdefs)))
