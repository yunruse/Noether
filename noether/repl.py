...
def rep(lang, lineno, globals_, locals_):
    """Iterative step of the REPL, handling errors, printing, and syntax errors."""
    try:
        lines = get_input()
        ...
        return
    source = "\n".join(lines)
    filename = "<Noether#{}>".format(lineno)
    chars = len("\n".join(lines)) + 1
    linecache.cache[filename] = chars, 0, lines, filename

    try:
        code = lang(source, filename=filename, globals=globals_, locals=locals_)
...
def repl(lang=Python, **kw):
    ...
    while True:
        n += 1
        exitVal = rep(lang, n, globals_, locals_)
        if exitVal is None:
            continue
